...
const { build } = require('@off-site/prettier-config/builder')
...
class Prettier {
    ...
    boot() {
        ...
        getAllFilesSync(this.source).toArray().forEach(file => {
            this.run(file)
        })

        this.watch && watcher.subscribe(this.source, (error, events) => {
            this.enqueue(events)
        })
    }
